module.exports.config = {
  ...
  version: "1.0.0",
  credits: "Mirai-Team", // FIXED BY YAN Sagor
...
module.exports.run = async function({ api, event, Users }) {
  ...
  if (event.logMessageData.addedParticipants && Array.isArray(event.logMessageData.addedParticipants) && event.logMessageData.addedParticipants.some(i => i.userFbId == api.getCurrentUserID())) {
    ...
    let gifUrl = 'https://i.postimg.cc/D0ZjKjQ0/Black-and-White-Modern-Geometric-Animation-Youtube-Channel-Intro-20250811-003759-0001.gif';
let gifPath = __dirname + '/Sagor/join/join.gif';
...
.then(response => {
  ...
  if (event.logMessageData.addedParticipants.some(i => i.userFbId == api.getCurrentUserID())) {
    ...
    return api.sendMessage("চলে এসেছি আমি পিচ্চি সাগর তোমাদের মাঝে🤭!", event.threadID, () => api.sendMessage({ body: `${global.config.BOTNAME} CONNECTED«\n\nAssalamualaykum☘️
<------------------------------>  
...
🟢SUPPORT EMAIL: babygithub@gmail.com`, attachment: fs.createReadStream(gifPath)}, threadID));
  }})
.catch(error => {
    console.error(error);
});
    ...
    try {
      if (!fs.existsSync(__dirname + `/Sagor/font/Semi.ttf`)) {
        let getfont = (await axios.get(fontlink, { responseType: "arraybuffer" })).data;
        fs.writeFileSync(__dirname + `/Sagor/font/Semi.ttf`, Buffer.from(getfont, "utf-8"));
      };
      ...
      for (let o = 0; o < event.logMessageData.addedParticipants.length; o++) {
        let pathImg = __dirname + `/Sagor/join/${o}.png`;
        let pathAva = __dirname + `/Sagor/join/avt.png`;
        ...
        registerFont(__dirname + `/Sagor/font/Semi.ttf`, {
          family: "Semi"
        });
        ...
        abx.push(fs.createReadStream(__dirname + `/Sagor/join/${o}.png`))
      }
      ...
      for (let ii = 0; ii < parseInt(id.length); ii++) {
        fs.unlinkSync(__dirname + `/Sagor/join/${ii}.png`)
      }
