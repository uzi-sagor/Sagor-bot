...
try{
 ...
 if(body.startsWith("বাবু") || body.startsWith("bby") || body.startsWith("বট") || body.startsWith("Bbz") || body.startsWith("bot")){
 const arr = body.replace(/^\S+\s*/, "")
 if(!arr) {
 await api.sendMessage("অভিনয় এর জন্য তুমি সেরা রে ...!!😫🐸 ", event.threadID, (error, info) => {
 global.client.handleReply.push({
 name: this.config.name,
 type: "reply",
 messageID: info.messageID,
